...
func TestDenomMsgServerCreate(t *testing.T) {
	...
	manager := "A"
	for i := 0; i < 5; i++ {
		expected := &types.MsgCreateDenom{Manager: manager,
		...
		require.Equal(t, expected.Manager, rst.Manager)
	}
}
...
func TestDenomMsgServerUpdate(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc: "Completed",
			request: &types.MsgUpdateDenom{Manager: manager,
				Denom: strconv.Itoa(0),
			},
		},
		{
			desc: "Unauthorized",
			request: &types.MsgUpdateDenom{Manager: "B",
				Denom: strconv.Itoa(0),
			},
