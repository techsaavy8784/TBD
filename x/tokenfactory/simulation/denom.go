...
func SimulateMsgUpdateDenom(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		for _, obj := range allDenom {
			simAccount, found = FindAccount(accs, obj.Manager)
			if found {
				denom = obj
				break
			}
		...
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "denom manager not found"), nil, nil
		}
		msg.Manager = simAccount.Address.String()
